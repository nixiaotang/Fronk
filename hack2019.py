...
def collect(ID, w, h, x, y) :
    ...
    if itSiz.collidepoint(mx, my) and mp[0] :
...
def arrow(x, y, direc, nextScene) :
    ...
    if direc == 'right' :
        ...
        rects = [Rect(x, y, 10, 100), Rect(x+10, y+10, 10, 80), Rect(x+20, y+20, 10, 60), Rect(x+30, y+30, 10, 40), Rect(x+40, y+40, 10, 20)]
    elif direc == 'left' :
        arw = image.load("arrow-left.png")
        arw = transform.scale(arw, (50, 100))
        rects = [Rect(x+40, y, 10, 100), Rect(x+30, y+10, 10, 80), Rect(x+20, y+20, 10, 60), Rect(x+10, y+30, 10, 40), Rect(x, y+40, 10, 20)]
    elif direc == 'up' :
        arw = image.load("arrow-up.png")
        arw = transform.scale(arw, (100, 50))
        rects = [Rect(x, y+40, 100, 10), Rect(x+10, y+30, 80, 10), Rect(x+20, y+20, 60, 10), Rect(x+30, y+10, 40, 10), Rect(x+40, y, 20, 10)]
    elif direc == 'down' :
        arw = image.load("arrow-down.png")
        arw = transform.scale(arw, (100, 50))
        rects = [Rect(x, y, 100, 10), Rect(x+10, y+10, 80, 10), Rect(x+20, y+20, 60, 10), Rect(x+30, y+30, 40, 10), Rect(x+40, y+40, 20, 10)]
...
def menu () :
    ...
    if button.collidepoint(mx, my) and mp[0]:
        scene = "Bedroom"
...
def bedroom() :
    ...
    if not oneClick[0] :
        screen.blit(bkg, (0, 0))
        if pnt.collidepoint(mx, my) and mp[0]:
            oneClick[0] = True
...
def kitchen() :
    ...
    if not oneClick[2] :
        ...
        if frig.collidepoint(mx, my) and mp[0] :
            oneClick[2] = True
    ...
    if not oneClick[1] :
        ...
        if len(inventories) > 0 and cab.collidepoint(mx, my) and mp[0] and inventories[page*3+selected] == 4:
            oneClick[1] = True
    ...
    if len(inventories) > 0 and ovenCol.collidepoint(mx, my) and mp[0] and inventories[page*3+selected] == 2:
        inventories.remove(inventories[page*3+selected])
        collected[0] = 0;
...
def livingRoom ():
    ...
    if len(inventories) > 0 and monstSiz.collidepoint(mx, my) and mp[0]:
